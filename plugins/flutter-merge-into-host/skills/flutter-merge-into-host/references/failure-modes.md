...
## Phase 3 (Adapt) — Rule 6 detection bias

- ✗ Using **filename as the trigger or skip signal** for Rule 6 (Category-list → PageView). Real-world miss: a page called `category_page.dart` was correctly grep-flagged in Phase 0 (tab/chip + masonry list + no PageView), but auto-mode applier rationalized a skip as "preserving source's original structure to minimize churn." This is not in the rule's skip-conditions list; it's just reluctance disguised as prudence. Symmetric anti-pattern: assuming a page is NOT a candidate because its filename doesn't contain "category" / "feed" / "list" — e.g. `discover.dart` / `index.dart` with the same widget combo is just as much a candidate.

  **正解**: Rule 6 detection is purely by widget-tree signals (`SwitchTab` / `TabBar` / `FilterChip` / `ChoiceChip` / `SegmentedButton` for tabs; `SliverGrid` / `SliverList` / `SliverMasonryGrid` / `GridView` / `ListView` for the main list; absence of `PageView` / `TabBarView`). Filename is metadata, not signal. After the grep produces the candidate set, the only valid escape hatches are the three documented skip conditions (list too short to scroll / single-toggle on-off / settings-style organizational tabs). If the candidate doesn't hit any of those, the transform runs.

- ✗ Skipping Rule 6 because the source's chip row "doesn't actually filter the data yet" (data is hardcoded global list, all chips render the same content). The rule is about preparing the UX shape ahead of the data; future API integration will swap the data source page-by-page. Skipping reverses the intent: when the API lands, someone has to remember to come back and add PageView + linkage manually. The transform should run now with a `// TODO: per-pageIndex data once API ready` comment at the `SliverChildBuilderDelegate`.

- ✗ Skipping Rule 6 because the source has a custom NestedScrollView + SliverPersistentHeader + ScrollController setup that "looks similar to PageView linkage". They're NOT similar — the source's pattern animates the chip-row scroll position but the content list never changes per-chip. Rule 6 transforms the content list axis from "1 list + chip filter" → "N lists in PageView + chip-driven page swap"; the SliverPersistentHeader becomes the NestedScrollView's outer header (wrapped in SliverOverlapAbsorber), inner pages each get their own CustomScrollView with PageStorageKey. The visual difference: with Rule 6 applied, swiping left/right on the content area changes the active chip. Without it, swiping does nothing.
...
- ✗ Leaving source's splash page intact as a reachable route (e.g. `/<src-name>/splash`) when the host has a central splash module (typically `module_base_page` or similar exporting `BasePageRouteConstant.splash` + `buildBasePageRoutes()`). Symptom is invisible during compile — both splashes work in isolation — but the source splash bypasses host's agreement gate / device-register / token-bootstrap / launch-ad pipeline. The user never sees the agreement screen (legal exposure), API calls 401 because no token bootstrap ran, ad revenue evaporates. **正解**: Phase 3 Rule 7 detects this, removes the source splash route, redirects launch to host's splash, then either replaces logo (Path A) or injects the source's visual widget via `splashBackground` (Path B).

- ✗ Collapsing source's frame-animation / video splash to a single static logo via `splashLogoAsset` because the host's factory only exposes that one parameter. Loses the designer's intended animation; user sees a frozen frame instead of the cute_animal-style 5-frame walk-in. The host's `SplashPage` often already has a `Widget? backgroundChild` parameter that the factory just doesn't pass through — a 3-line factory edit unlocks the full visual. **正解**: Phase 3 Rule 7 Step 1 classifies host into L0/L1/L2 by widget-injection capability; for L1 (param exists in SplashPage but not in factory), Step 6 adds the pass-through. For L0, Step 6 also adds an integration-guide section so the change is documented for other consumers. Then Path B preserves the animation.

- ✗ Extracting source splash into a content widget for Path B but forgetting to strip the source's lifecycle/navigation/scaffold code. The injected widget then double-Scaffolds (host wraps it in `Scaffold(backgroundColor: Colors.black)`, the source widget wraps again), races the host's `context.go` with its own `Timer.then.go`, sets its own `SystemChrome` style that conflicts with host's — splash visually flickers or jumps to home before agreement dialog. **正解**: Rule 7 Step 4's strip list — remove `Scaffold`, `context.go/push`, `WidgetsBindingObserver`, `SystemChrome.set*`, business init. Keep only the pure visual (Image / VideoPlayer / Lottie / animation controllers).
